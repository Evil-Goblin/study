...
하지만 이 코드도 문제가 있는데, `isRomanNumeral` 메소드가 호출되지 않는다면, `ROMAN`이라는 필드를 필요없이 만든셈이 된다. 게으른 초기화(lazily initializing)([아이템83](item83.md))를 사용해서 최적화 할 수 있지만 추천하진 않는다. 보통 지연 초기화는 측정 가능한 성능 개선 없이 구현을 복잡하게 만든다.([아이템67](item67.md))
